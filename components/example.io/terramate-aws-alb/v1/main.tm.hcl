generate_hcl "main.tf" {
  lets {
    alb_bundle_uuid = tm_try(component.input.tags.value["example.io/bundle-uuid"], null)

    # ECS service bundles that target this ALB (by bundle uuid)
    ecs_service_bundles = [
      for svc in tm_bundles("example.io/tf-aws-ecs-fargate-service/v1") :
      ...
      attributes = {
        ...
        tags = merge(
          component.input.tags.value,
          {
            ...
            "example.io/target-group-for-bundle-uuid" = resource.key
