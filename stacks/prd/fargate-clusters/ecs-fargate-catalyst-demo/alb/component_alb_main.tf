...
data "aws_vpc" "vpc_by_tags" {
  ...
  filter {
    ...
    values = [
      "c2a0ddaf-fdd6-4b4a-88d1-9f4c5273e7bf",
    ]
  }
}
...
module "alb" {
  ...
  listeners = {
    http = {
      ...
      protocol = "HTTP"
      rules = {
        ecs-fargate-catalyst-demo-nginx-service = {
          actions = [
            {
              forward = {
                target_group_key = "ecs-fargate-catalyst-demo-nginx-service"
              }
            },
          ]
          conditions = [
  ...
  target_groups = {
    ecs-fargate-catalyst-demo-nginx-service = {
      ...
      name        = "ecs-fargate-catalyst-demo-nginx-"
      port        = 80
      protocol    = "HTTP"
      target_type = "ip"
    }
