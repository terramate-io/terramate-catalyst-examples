...
module "alb" {
  ...
  listeners = {
    http = {
      ...
      protocol = "HTTP"
      rules = {
        demo-cluster-my-first-service = {
          actions = [
            {
              forward = {
                target_group_key = "demo-cluster-my-first-service"
              }
            },
          ]
          conditions = [
        ...
        demo-cluster-my-second-service = {
          actions = [
            {
              forward = {
                target_group_key = "demo-cluster-my-second-service"
              }
            },
          ]
          conditions = [
  ...
  target_groups = {
    ...
    demo-cluster-my-second-service = {
      ...
      health_check = {
        ...
        interval            = 30
        matcher             = "200"
        path                = "/my-second-service/"
        port                = "traffic-port"
        protocol            = "HTTP"
        timeout             = 5
        unhealthy_threshold = 2
      }
      name        = "demo-cluster-my-second-service"
      port        = 3000
      protocol    = "HTTP"
      target_type = "ip"
    }
